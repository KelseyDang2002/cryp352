...
# Create a socket
serverSock = socket.socket(socket.AF_INET, socket.SOCK_STREAM) # IPv4 and TCP
...
while True:
	...
	srvMsg = str(cliMsg.decode()).upper()

	print("Client sent " + srvMsg)
 	
  	# Send back the uppercase version of the message
	# serverSock.send(srvMsg.encode())
